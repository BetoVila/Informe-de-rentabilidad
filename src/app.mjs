...
function telemetryPanel(){
 ...
 return `<section class="panel"><h2>Medido por el camión (Movertis)</h2><p class="sub">Km y litros medidos por el propio camión, del ${date(v.from)} al ${date(v.to)}. Solo días con lectura fiable: un día sin lectura no cuenta como cero. El ERP empezó a bajarlos el ${date(m.desde)}; el histórico anterior aún no está cargado.</p><div class="kpi-grid">${[
  ['Km medidos',nf(v.km),nf(v.plates)+' camiones activos'],['Litros medidos',nf(v.litres),'Camiones con sensor de combustible'],['Consumo medido',nf(v.consumption,1)+' l/100 km','Litros / km de los camiones con sensor'],['Días-camión activos',nf(v.activeDays),'Circula 30 km o más ese día'],['Días activos sin parte',nf(v.daysWithoutPart)+' ('+pct(v.pctWithoutPart)+')','El camión circuló y no hay parte de Access'],['Km sin parte',nf(v.kmWithoutPart),'Kilómetros de esos días'],['Km del parte / km medidos',nf(v.ratio,2),'En los días con parte (1 = coinciden)']
...
function renderContent(){
 ...
 if(state.tab==='summary'&&ledgerCtx.ledgerOn){
   ...
   html=partialWarn()+panel(state.tab==='plate'?'Vehículos con más ingresos':'Clientes con más ingresos','Primeros ocho por ingresos; tabla completa debajo.',bars(rows.slice(0,8),'revenue'));
...
function personalRows(){
 ...
 for(const p of P.people){
  ...
  out.push({key:p.key,name:p.name,company:main.company,typeLabel:t.label+(t.unsure?' (por confirmar)':''),type:t.type,cost,devengos:total('devengos'),ss:total('ss'),dietas:total('dietas'),hours,perHour:hours>0?cost/hours:null,accessHour:access,gap:hours>0&&access?cost/hours-access:null});
 }
...
function personalView(){
 ...
 const table=setTable('Personas','«€/hora real» = coste de empresa del mes ÷ horas de sus partes. «€/hora en Access» es la tarifa del contrato que usan los partes: la diferencia dice qué tarifas hay que corregir. Bajas, vacaciones y personas sin partes aparecen igual, con las horas en blanco.',rows,[{label:'Persona',key:'name'},{label:'Trabajo que realiza',key:'typeLabel'},{label:'Empresa',key:'company'},moneyCol('Coste empresa','cost'),moneyCol('Devengos','devengos'),moneyCol('Seg. Social','ss'),moneyCol('Dietas','dietas'),numberCol('Horas en partes','hours',1),moneyCol('€/hora real','perHour'),moneyCol('€/hora en Access','accessHour'),moneyCol('Diferencia','gap')]);
