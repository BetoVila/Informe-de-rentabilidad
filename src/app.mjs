...
function drawTable(){
 ...
 $('tableArea').innerHTML=count?`<div class="tablewrap"><table><thead><tr>${def.columns.map(c=>`<th scope="col" aria-sort="${tableState.sort===c.key?(tableState.asc?'ascending':'descending'):'none'}"><button data-sort="${c.key}">${esc(c.label)} ${tableState.sort===c.key?(tableState.asc?'↑':'↓'):'↕'}</button></th>`).join('')}</tr></thead><tbody>${view.map(r=>`<tr>${def.columns.map((c,i)=>`<td class="${c.numeric?'num':''} ${signedTone(c,r)||(c.numeric&&r[c.key]<0?'neg':'')}" title="${esc(c.format?c.format(r[c.key]):r[c.key])}">${i===0&&def.drill?`<button class="tablelink" data-drill="${def.drill}" data-key="${esc(r.key)}">${esc(r[c.key])} ↗</button>`:esc(c.format?c.format(r[c.key]):r[c.key])}</td>`).join('')}</tr>`).join('')}</tbody></table></div><div class="pager"><span>${tableState.page*50+1}–${Math.min((tableState.page+1)*50,count)} de ${nf(count)} · todas las filas están disponibles</span><div><button data-page="-1" ${tableState.page===0?'disabled':''}>← Anterior</button><button data-page="1" ${(tableState.page+1)*50>=count?'disabled':''}>Siguiente →</button></div></div>`:`<div class="empty">No hay datos para esta combinación. Cambie los filtros o el texto de búsqueda.</div>`;
}
