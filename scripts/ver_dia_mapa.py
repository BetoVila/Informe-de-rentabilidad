...
<div class="tlwrap"><svg id="tl"></svg><p class="tlcap"><span>Línea de tiempo del día (hora de Madrid): arriba cada viaje con su número; debajo las paradas (verde cargando, azul descargando, naranja espera dentro de un viaje, gris fuera de viaje); la barra fina es la jornada. Pincha un viaje para resaltarlo.</span></p></div>
...
<div class="tw"><table><thead><tr><th>Nº</th><th>Albarán cantera</th><th>Origen → destino</th><th>Carga (llega → sale)</th><th>Descarga (llega → sale)</th><th>Inicio → fin del viaje</th><th class="num">Km</th><th class="num">Km carg. / vacío</th><th class="num">Litros</th><th class="num">Min</th><th class="num">Conduc.</th><th class="num">Espera</th><th>Chofer</th><th>Método / conf.</th><th>Aviso</th></tr></thead><tbody id="tvb"></tbody></table></div>
<h2>Paradas y esperas del día</h2>
<div class="tw"><table><thead><tr><th>Hora</th><th class="num">Min</th><th>Lugar</th><th>Viaje</th><th>Qué hacía</th></tr></thead><tbody id="tpb"></tbody></table></div>
</div><script>
const META=@@META@@,V=@@V@@,L0=@@L@@,P=@@P@@,J=@@J@@;
...
$('chips').innerHTML=[['Albaranes',META.n],['Viajes medidos',META.med],['Km del día (medidos)',f(META.km)],['Litros (medidos)',f(META.lit,1)],['Jornada',META.jor]].map(([k,v])=>`<span class="chip"><small>${k}</small><b>${v}</b></span>`).join('');
...
for(const v of V){if(!v.n)continue;const pc=v.posc||v.pos0;if(pc){const m=L.marker(pc,{icon:L.divIcon({className:'',html:`<div class="nm" style="background:${v.col}">${v.n}</div>`,iconSize:[22,22],iconAnchor:[11,11]}),zIndexOffset:1000}).addTo(map).bindTooltip(`Viaje ${v.n}: carga en ${v.on}`).on('click',()=>sel(v.n));m.n=v.n;marks.push(m);}
 if(v.posd){const m=L.marker(v.posd,{icon:L.divIcon({className:'',html:`<div class="nm sq" style="background:${v.col}">${v.n}</div>`,iconSize:[22,22],iconAnchor:[11,11]}),zIndexOffset:1000}).addTo(map).bindTooltip(`Viaje ${v.n}: descarga en ${v.dn}`).on('click',()=>sel(v.n));m.n=v.n;marks.push(m);}}
if(b.length)map.fitBounds(b,{padding:[20,20]});
$('leg').innerHTML=V.filter(v=>v.n).map(v=>`<button class="lg" data-n="${v.n}"><b style="background:${v.col}"></b>${v.n} · ${v.on} → ${v.dn}${v.mas1?' (+1)':''}</button>`).join('');
function timeline(){const svg=$('tl');const W=Math.max(600,svg.clientWidth||1000);svg.setAttribute('viewBox',`0 0 ${W} 104`);
 const med=V.filter(v=>v.ini_s);const t0=Math.min(...J.map(j=>j[0]),...med.map(v=>v.ini_s))-900,t1=Math.max(...J.map(j=>j[1]),...med.map(v=>v.fin_s))+900;const X=t=>W*(t-t0)/(t1-t0);let s='';
 ...
 for(const j of J)s+=`<rect x="${X(j[0])}" y="84" width="${Math.max(1,X(j[1])-X(j[0]))}" height="6" fill="#c7d7f0" rx="2"><title>jornada ${hm(j[0])} → ${hm(j[1])} (${j[3]} h${j[2]?', nocturna':''})</title></rect>`;
 for(const v of med){const x=X(v.ini_s),w=Math.max(2,X(v.fin_s)-x);s+=`<g class="blk" data-n="${v.n}"><rect x="${x}" y="12" width="${w}" height="32" fill="${v.col}" rx="3"/>${w>16?`<text x="${x+w/2}" y="33" text-anchor="middle" font-size="12.5" font-weight="700" fill="#fff">${v.n}</text>`:''}<title>Viaje ${v.n}: ${v.on} → ${v.dn} · ${hm(v.ini_s)} → ${hm(v.fin_s)}${v.km!=null?' · '+f(v.km)+' km':''}</title></g>`;}
 for(const p of P){const x=X(p[2]),w=Math.max(1.5,X(p[3])-x);s+=`<rect x="${x}" y="52" width="${w}" height="20" fill="${ROL[p[9]]}" rx="2"><title>${p[4]} · ${p[5]} min · ${p[8]||'lugar no conocido'}</title></rect>`;}
 svg.innerHTML=s;svg.querySelectorAll('.blk').forEach(g=>g.addEventListener('click',()=>sel(+g.dataset.n)));if(cur)marcar();}
function tablas(){$('tvb').innerHTML=V.map(v=>`<tr data-n="${v.n||''}" style="border-left:6px solid ${v.n?v.col:'#9ca3af'}"><td>${v.n||'—'}</td><td>${v.alb}</td><td>${v.on} → ${v.dn}</td><td>${v.tc?hm(v.tc)+' → '+hm(v.tcf):''}</td><td>${v.td?hm(v.td)+(v.tdf?' → '+hm(v.tdf):''):''}</td><td>${v.ini_s?hm(v.ini_s)+' → '+hm(v.fin_s)+(v.mas1?' (+1)':''):(v.met==='dia'?'repartido (sin hora)':'')}</td><td class="num">${f(v.km)}</td><td class="num">${v.kmc!=null?f(v.kmc)+' / '+f(v.kmv):''}</td><td class="num">${f(v.lit,1)}</td><td class="num">${f(v.min)}</td><td class="num">${f(v.cond)}</td><td class="num">${f(v.esp)}</td><td>${v.chofer||''}</td><td>${v.met||''}${v.conf?' / '+v.conf:''}</td><td class="aviso">${v.aviso||''}</td></tr>`).join('');
 $('tpb').innerHTML=P.map(p=>`<tr data-n="${p[7]||''}"><td>${p[4]}</td><td class="num">${p[5]}</td><td>${p[8]||'<i>lugar no conocido</i>'}</td><td>${p[7]?'<b style="color:'+p[6]+'">viaje '+p[7]+'</b>':'—'}</td><td><span class="rol" style="background:${ROL[p[9]]}"></span>${ROLTXT[p[9]]}</td></tr>`).join('');
...
function sel(n){cur=(!n||n===cur)?0:n;
 ...
 if(cur){const pts=[];for(const pl of lines)if(pl.n===cur)pts.push(...pl.getLatLngs());if(pts.length)map.fitBounds(pts,{padding:[30,30]});}else if(b.length)map.fitBounds(b,{padding:[20,20]});}
$('todos').onclick=()=>sel(0);
...
def render_dia(mat, fecha, viajes, pts_mat, dg_dia, rest_s, paso_s, lug=None):
    ...
    for x in viajes:
        ...
        V.append({"n": n if medido else None, "od": x.get("orden_dia"), "col": col, "alb": x["cantera"], "o": x["origen"], "d": x["destino"],
                  ...
                  "pos0": pos_at(ini_s), "posc": pos_at(tc) if tc else None, "posd": pos_at(td) if td else None})
    ...
    # paradas: viaje al que pertenecen y que hacia (carga / descarga / espera / fuera), con el lugar conocido mas cercano
    P = []
    ...
    for p in par:
        ...
        for v in vmed:
            if v["ini_s"] <= p["t_in"] <= v["fin_s"] or v["ini_s"] <= p["t_out"] <= v["fin_s"]:
                col, k = v["col"], v["n"]
                if v["tc"] and p["t_in"] <= (v["tcf"] or v["tc"]) and p["t_out"] >= v["tc"]:
                    rol, lugar = "carga", v["on"]          # la parada de carga ES el origen del viaje (deducido del cruce)
                elif v["td"] and p["t_in"] <= (v["tdf"] or v["td"]) + 60 and p["t_out"] >= v["td"]:
                    rol, lugar = "descarga", v["dn"]
                else:
                    rol = "espera"
                break
        P.append([round(p["lat"], 5), round(p["lon"], 5), p["t_in"], p["t_out"], t2.iso_min(p["t_in"])[11:] + "–" + t2.iso_min(p["t_out"])[11:],
                  int((p["t_out"] - p["t_in"]) // 60), col, k, lugar or lug.cerca(p["lat"], p["lon"]), rol])
    ...
    meta = {"mat": mat, "fecha": fecha, "n": len(viajes), "med": len(vmed), "km": round(sum(v["km"] or 0 for v in vmed)), "lit": round(sum(v["lit"] or 0 for v in vmed), 1), "jor": jtxt}
    ...
    sub = ("Traza real del localizador. Cada color es un viaje medido del triangulado (de su hora de inicio a la de fin); gris = sin viaje asignado. "
           "Jornada(s): %s. Hora de Madrid. El primer viaje del día arranca al primer movimiento (incluye la ida desde la base); el último llega al fin de la jornada (incluye la vuelta). "
           "Los viajes «repartidos» no tienen hora: su km y minutos son un reparto de los ciclos sobrantes del día.") % jtxt
