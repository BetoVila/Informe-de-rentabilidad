...
if(actividadSrc?.metadata?.disponible){
 ...
 const rows=actividadSrc.rows.map(r=>{
  ...
  return [c,mi,ci,mti,oi,di,li,ld,r.km||0,r.m3||0,r.t||0,r.imp||0,r.horm?1:0,po,pd,r.kmr||0,r.lit||0,r.dur||0,TRM[r.trm]??3,r.impro||0,dd];
 });
 ...
 actividad={meta:{fuente:actividadSrc.metadata.fuente,desde:actividadSrc.metadata.desde,hasta:actividadSrc.metadata.hasta,viajes:rows.length,leido:actividadSrc.metadata.leido},co,mo,cli,mat,prov,loc,pt,dia,coords:actividadSrc.metadata.coords||{},rows,margen};
}
