...
- **`_reparto(f)` (línea 310) — el corazón de la conciliación.** Reparte el gasto del libro a los viajes **MES A MES**:
  ...
  - **Regla de la subcontrata (Roberto, 24/09/2026): «se subcontrata parcial, se pasan los ingresos y los gastos que corresponde, por tiempo, km o pactado a la subcontrata y el resto no».** A la subcontrata se le pasa su parte, que es lo pactado (`IMPPRO`); el resto del viaje es nuestro. Viaje con `IMPPRO > 0` (`sub`): coste = `IMPPRO` reescalado a la 607 + su parte de indirectos + **el material que vende ese viaje**, sin gasoil, conductor ni flota propios. Comprobado con los datos el 24/09: en todos los viajes con `IMPPRO > 0` el camión no es de la empresa del albarán; es de fuera o, en Agetrans, de Razo (dueño formal por `vehicu.dbf` sin proveedor). No hay subcontrata parcial con camión propio de la misma empresa. Si apareciera, lo propio (medido por el localizador o, si no, por el ingreso que queda = facturado − `IMPPRO`) se sumaría a `IMPPRO`. **No hay umbral** (el 40 % de tarifas no está en la regla).
  - **Material de los subcontratados (desde el 24/09).** El material del cliente y mes (`inggas`, escalado a los áridos del libro) se reparte entre sus viajes propios (por ingreso, como siempre) y sus subcontratados que llevan toneladas. La parte de los subcontratados va por sus toneladas frente a las de los propios; si los propios no llevan toneladas, por el ingreso que nos queda (facturado − `IMPPRO`) frente al de los propios. Dentro de los subcontratados va por tonelada (`sRate`), y dentro de los propios por ingreso (`dRate`). Antes los subcontratados llevaban material 0 y su material caía en los viajes propios del cliente. Efecto en Razo: 666 viajes con 35.106 EUR en 2025 y 826 viajes con 31.189 EUR en 2026 (sobre todo RODIÑAS PRETENSADO y GENERAL DE HORMIGONES), con el mismo €/t que los propios del cliente. Los totales del mes no cambian.
